...
nbawide <- nba |> pivot_wider(names_from = Season, values_from = OffRtg)

tops <- nbawide |>
  mutate(Difference = `2021-2022` - `2020-2021`) |>
...
nba |> pivot_wider(names_from = Season, values_from = OffRtg)
```
...
Then, we're going to use mutate to subtract last season's offensive rating -- the points per 100 possessions -- from this season's. **A quirk of dplyr: Because the column names now start with a number -- for example 2019 -- we have to put backticks around the seasons. The backtick says "this is okay."
