---
title: "Advanced Sports Data Lesson 6: Joins"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
home <- home_games %>% inner_join(teamstats, by=c("id"="game_id", "home_uid"="team_uid")) %>% rename(home_team_short_display_name = team_short_display_name, home_team_score=team_score, home_rolling_shooting_percentage = rolling_shooting_percentage)

away <- away_games %>% inner_join(teamstats, by=c("id"="game_id", "away_uid"="team_uid")) %>% rename(away_team_short_display_name = team_short_display_name, away_team_score=team_score, away_rolling_shooting_percentage = rolling_shooting_percentage)
```
...
If you explore around in that data, you'll see two different kinds of IDs. There's columns that have XXXX.id after them, and some have XXXX.uid after them. The difference is not really that important -- it's unique to the data provider -- but both represent ID numbers for that thing. For example `home_uid` and `away_uid` represent the id numbers of the home and away team. The `id` column is the ID number for this particular game. If we use both, we can get the home team for that specific game -- meaning the home team at that moment. That's important.
...
This sometimes take a little bit of deduction, but if you look, you'll see `game_id`, which appears to match the `id` from the schedule dataset. Because we only have one team here, we won't have `home_uid` or `away_uid` ... but we do have `team_uid`, which appears to match the form of the schedule uids. 
...
### Exercise 1: Find which uid you need
...
### Exercise 2: What kind of join?

Here's what that looks like:

```{r join3, exercise=TRUE, exercise.setup = "join-load-data", message=FALSE}
home_games %>% 
  ?????_join(teamstats, by=c("id"="game_id", "home_uid"="team_uid")) %>%
  rename(
    ????_team_short_display_name = team_short_display_name, 
    ????_team_score=team_score, 
    ????_rolling_shooting_percentage = rolling_shooting_percentage
    )
```
```{r join3-solution, exercise.reveal_solution = FALSE}
home_games %>% 
  inner_join(teamstats, by=c("id"="game_id", "home_uid"="team_uid")) %>%
  rename(
...
Behind the scenes, I've gone ahead and made a `home` and `away` dataframe that is the result of the above code. It does what you did, just stores them in a variable called home and away. The away one renamed everything the way you think it would. 
